                        ...
                        <v-row>
                            <v-col cols="12">
                                <v-combobox
                                    ...
                                    dense
                                    outlined
                                    :menu-props="{ maxHeight: '200px', attach: true }"
                                ></v-combobox>
                            </v-col>
                        </v-row>
                        <v-row>
        ...
        handle_submit() {
            ...
            if (
                ...
                axios.post('/api/develop/gitPr', {
                    ...
                    body: this.html,
                    base: this.dstBranch,
                    labels: this.selectedLabels // 使用选择的标签
                }).then( (res) => {
                    if (res.data.errcode === 0) {
                        ...
                        this.$message.error({
                            labels: 'error',
                            message: '提交PR失败: ' + res.data.message
                        });
                    }
                }).catch((err) => {
                    this.$message.error({
                        labels: 'error',
                        message: '提交PR失败: ' + err.message
                    });
                });
            } else {
                this.$message.error({
