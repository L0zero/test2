...
def GetDocContent(userId, projectId, docId):
    userProject = isUserInProject(userId, projectId)
    docEntry = Document.objects.get(id=docId)
    response = {
        'message': "get userDocContent ok",
        'errcode': 0,
        'data': {
...
class UserGetDocContent(View):
    def post(self, request):
        ...
        try:
            response = GetDocContent(kwargs.get('userId'), kwargs.get('projectId'),
                                         kwargs.get('docId'))
...
def userDocListTemplate(userId, projectId, table):
    ...
    tableEntries = table.objects.all()
    print("tableEntries", tableEntries)
...
def userCreateDoc(userId, projectId, name, outline, content, accessUserId):
    ...
    #doc_db = VectorDBManager(name=f"{projectId}_doc")
    #doc_db.save_text_to_db([content])
