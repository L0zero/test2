...
export default {
  ...
  methods: {
    ...
    openForm() {
      ...
      axios.post('api/ai/generateCommitMessage', {
        userId: this.user.id,
        projectId: this.projectId || this.proj?.projectId,
        repoId: this.repoId || this.$route.params.repoid,
  ...
  created() {
    this.fileTreeReady = false;
    console.log("file projectId = " + (this.projectId || this.proj?.projectId))
    console.log("file userId = " + this.user.id)
    console.log("file repoId = " + (this.repoId || this.$route.params.repoid))
...
<template>
  <v-container>
    ...
    <v-row style="margin-top: 20px">
      <v-col cols="3" class="file-tree-col">
        <div class="tabs-menu">
          <h2>文件树 - {{ branchName }}</h2>
        </div>
      ...
      <v-col :cols="fileContentReady ? 7 : 9" class="editor-col">
      ...
      <v-col cols="2" v-if="fileContentReady" class="assistant-col">
...
.assistant-col .v-card {
  font-size: 0.9em;
}

.assistant-col .v-card-title {
  font-size: 1.1em;
  padding: 8px 16px;
}

.assistant-col .v-card-text {
  padding: 8px 16px;
}
