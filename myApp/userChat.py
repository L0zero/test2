...
def check_user_access_to_content_project(request):
    ...
    if not UserProject.objects.filter(user_id=user_id, project_id=project.id).exists():
        return response_json(
            errcode=SUCCESS,
            data={'hasAccess': False}
        )
    
def check_user_access_to_content_task(request):
    kwargs: dict = json.loads(request.body)

    user_id = int(kwargs.get('userId'))
    task_id = int(kwargs.get('taskId'))
    ...
    if not UserTask.objects.filter(user_id=user_id, task_id=task.id).exists():
        return response_json(
            errcode=SUCCESS,
            data={'hasAccess': False}
        )
    
def check_user_access_to_content_document(request):
    kwargs: dict = json.loads(request.body)

    user_id = int(kwargs.get('userId'))
    document_id = int(kwargs.get('documentId'))
    ...
    if not UserDocument.objects.filter(user_id=user_id, document_id=document.id).exists():
        return response_json(
            errcode=SUCCESS,
            data={'hasAccess': False}
        )
    
def check_user_access_to_content_repo(request):
    kwargs: dict = json.loads(request.body)

    user_id = int(kwargs.get('userId'))
    project_id = int(kwargs.get('projectId'))
    repo_id = int(kwargs.get('repoId'))
    ...
    if not UserProjectRepo.objects.filter(user_id=user_id, project_id=project_id, repo_id=repo.id).exists():
        return response_json(
            errcode=SUCCESS,
            data={'hasAccess': False}
        )  
