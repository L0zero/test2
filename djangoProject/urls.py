...
from myApp import AIChat, AICompletion, userdevelop, manager, userBasic, userPlan, debug, AI, shareDoc, file, mail, version
from myApp import chatConsumer
from myApp import codeEditorConsumer

from myApp import notice, userChat, sonar
from django.urls import re_path
from myApp.ai import knowledgeBase
from myApp.bot import Bot

websocket_urlpatterns = [
    re_path(r"ws/chat/(?P<userId>\w+)/(?P<roomId>\w+)$",
            chatConsumer.ChatConsumer.as_asgi()),
    re_path(r'ws/editor/(?P<doc_id>\w+)$', codeEditorConsumer.CodeEditorConsumer.as_asgi()),
]
...
urlpatterns = [
    ...
    path('api/develop/createIssue', userdevelop.CreateIssue.as_view()),
    path('api/develop/resolvePr', userdevelop.ResolvePr.as_view()),
    path('api/develop/getPrDetails', userdevelop.GetPrDetails.as_view()),
    path('api/develop/getIssueDetails', userdevelop.GetIssueDetails.as_view()),
    path('api/develop/closeIssue', userdevelop.CloseIssue.as_view()),
    path('api/develop/updateIssue', userdevelop.UpdateIssue.as_view()),
    ...
    path('api/doc/userGetDocContent', shareDoc.UserGetDocContent.as_view()),
    ...
    path('api/chat/checkUserAccessToContentProject', userChat.check_user_access_to_content_project),
    path('api/chat/checkUserAccessToContentRepo', userChat.check_user_access_to_content_repo),
    path('api/chat/checkUserAccessToContentTask', userChat.check_user_access_to_content_task),
    path('api/chat/checkUserAccessToContentDocument', userChat.check_user_access_to_content_document),
    ...
    path('api/aiChat', AIChat.chat_ai_view),  # AI 聊天接口
    path('api/aiCompletion', AICompletion.ai_completion),  # AI 代码补全接口
    path('api/knowledgeBase/generateDiscussionSummary',knowledgeBase.GenerateDiscussionSummary.as_view()),
    path('api/knowledgeBase/askQuestion', knowledgeBase.AskQuestion.as_view()),
  
    path('api/version', version.Version.as_view()),
    
    path('api/sonar/measures/component', sonar.MeasuresComponent.as_view()),
    path('api/sonar/issues/search', sonar.IssuesSearch.as_view()),
    path('api/sonar/scan', sonar.Scan.as_view()),
    path('api/bot/config', Bot.as_view()),
]
