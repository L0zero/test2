          ...
          <v-tab-item v-for="repository in bindRepos" :key="repository.id">
              ...
              <v-row>
                <v-col cols="12">
                  <v-card class="mb-4">
                    <v-card-actions>
                      <v-btn :color="getTopicColor(user.topic)" @click="$router.push('/commitReview')">Commit评审</v-btn>
                      <v-btn :color="getTopicColor(user.topic)" @click="$router.push('/prReview')">PR评审</v-btn>
                      <v-btn :color="getTopicColor(user.topic)" @click="$router.push('/newPR')">新建PR</v-btn>
                      <v-btn :color="getTopicColor(user.topic)" @click="$router.push('/issues')">Issues</v-btn>
                    </v-card-actions>
                  </v-card>
                </v-col>
              </v-row>
