<script>
  ...
  export default {
    ...
    methods: {
      async prToChat() {
        try {
          const {name, description} = discussionService.generateRoomInfo('pr', {
            id: this.prId,
            title: this.prId
          });
      ...
      async confirmDiscussion() {
        try {
          ...
          const preMess = {
            type: this.discussionForm.type,
            projectName: this.prId,
            name: this.prId,
            str: `http://114.116.211.54/prReview/${this.projId}/${this.repoId}/${this.prId}?prId=${this.prId}&projId=${this.projId}&repoId=${this.repoId}`
          };
...
<div style="margin: 15px">
  ...
  <div style="margin: 10px 0 0 20px">
    ...
    <div style="display: inline-block; margin-top: -20px; padding: -20px; margin-bottom: -20px"  v-if="this.isProjectReviewer === true && this.prDetail.state === 'open'">
      ...
      <v-chip :color="'red'" dark style="margin-right: 30px" @click="reviewPr(0)">
        拒绝
      </v-chip>
      <v-chip :color="'blue'" dark @click="prToChat">
        讨论发起
      </v-chip>
    </div>
