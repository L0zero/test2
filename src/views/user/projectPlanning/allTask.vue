<template>
  ...
  <div v-else style="width: 100%; height: 100%;" >
    ...
    <div class="three" style="position: absolute; left: 5%; right: 5%; top: 80px; bottom: 0; overflow-y: auto; max-height: calc(100vh - 120px); background: #fff; box-shadow: 0 2px 8px rgba(0,0,0,0.12), 0 0 6px rgba(0,0,0,0.04); border-radius: 12px; border: 1px solid #e0e0e0; padding: 24px 16px 16px 16px;">
          ...
          >
            <template v-slot:no-data>
              <div style="text-align: center; margin-top: 40px;">
                <div style="font-size:20px;font-weight: bold;text-align: center; color: #b0b0b0;">
                  您还没有设置冲刺
                </div>
              </div>
            </template>
            ...
            <template v-slot:default="{ items }">
              <v-row>
                <v-col v-for="task in items" :key="task.taskId" cols="12">
                  <v-card style="width: 100%; margin-bottom: 20px;">
                    ...
                    >
                      ...
                      <template v-slot:foot="{ item }">
                        ...
                        <div style="position: absolute; left: 10%; bottom: 4%">
                          <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                              <v-btn icon v-on="on" @click="taskCreateDiscussion(task)">
                  ...
                  <v-card v-if="task && task.displayReviews" style="position: relative; margin-top: 2px; width: 100%">
    ...
    <el-dialog title="发起讨论" :visible.sync="discussionDialog" width="50%" :before-close="handleDiscussionClose">
      <el-form :model="discussionForm" ref="discussionForm" label-width="100px">
        <el-form-item label="讨论室名称">
          <el-input v-model="discussionForm.name"></el-input>
        </el-form-item>
        <el-form-item label="讨论室简介">
          <el-input type="textarea" :rows="3" v-model="discussionForm.description"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="discussionDialog = false">取 消</el-button>
        <el-button type="primary" @click="confirmDiscussion">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import discussionService from "@/utils/discussion";//讨论发起功能
...
export default {
  ...
  methods: {
    ...
    async initiateDiscussion() {
      try {
        const {name, description} = discussionService.generateRoomInfo('project', {
          id: this.selectedProj.projectId,
          title: this.selectedProj.projectName
        });
        ...
        this.$message.error('准备讨论室信息失败：' + err.message);
      }
    },
    async taskCreateDiscussion(task) {
      try {
        const {name, description} = discussionService.generateRoomInfo('task', {
          id: task.taskId,
          title: task.taskName
        });
    ...
    async confirmDiscussion() {
      try {
        const roomId = await discussionService.createDiscussion({
          type: this.discussionForm.type,
          projectId: this.selectedProj.projectId,
          user: this.user,
          context: this.discussionForm.context,
          name: this.discussionForm.name,
          description: this.discussionForm.description,
          members: this.discussionForm.members
        });

        const preMess = {
          type: this.discussionForm.type,
          projectName: this.selectedProj.projectName,
          name: this.discussionForm.context.title,
          str: 'http://114.116.211.54/allTask'
        };
        
        const mess = discussionService.generateInitMessage(preMess);
        ...
        this.$message.error('讨论室创建失败：' + err.message);
      }
    },
    handleDiscussionClose(done) {
