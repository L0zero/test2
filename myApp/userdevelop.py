...
from myApp.service.git import GithubRepo

repo_semaphore = {}
os.environ['GH_TOKEN'] = 'ghp_123456'  # TODO 这都是什么东西 
...
def needRefresh(token, repo):
    ...
    try:
        ...
        git = GithubRepo(token, remotePath, localPath) # 创建GithubRepo对象
        status = git.check_local_branches_status() # 检查本地分支状态
        if True in status.values():
            return True
        elif None in status.values():
            # 如果有分支不存在，则需要更新
            return True
        return False
    except Exception as e:
...
class CheckRefreshRepo(View):
    def post(self, request):
        ...
        if needRefresh(token, repo) == True:
            response["data"] = {"needRefresh": True}
        else:
            response["data"] = {"needRefresh": False}
        return JsonResponse(response)
...
class RefreshRepo(View):
    def post(self, request):
        ...
        localPath = repo.local_path
        try:
            git = GithubRepo(token, repo.remote_path, localPath)  # 创建GithubRepo对象
            git.pull_all()  # 拉取所有分支
        except Exception as e:
    ...
    def post(self, request):
        ...
        try:
            ...
            for userProjectRepo in userProjectRepos:
                ...
                remote_path = userProjectRepo.repo_id.remote_path
                local_path = userProjectRepo.repo_id.local_path
                name = userProjectRepo.repo_id.name
                git = GithubRepo(token, remote_path, local_path)
                response["data"].append({"repoId": repoId,
                                         "repoRemotePath": remote_path,
                                         "name": name,
                                         "repoIntroduction": git.get_description()})
        except Exception as e:
            print(e)
            return JsonResponse(genUnexpectedlyErrorInfo(response, e))
...
class GetRepoAllFiles(View):
    def post(self, request):
        ...
        token = user.token
        try:
            git = GithubRepo(token, repo.remote_path, repo.local_path)  # 创建GithubRepo对象
            data = git.get_directory_structure(branch)  # 获取目录结构
        except Exception as e:
...
class GitPr(View):
    def post(self, request):
        ...
        try:
            ai_validation = self.validate_pr_with_ai(title, body, labels)
            final_labels = ai_validation.get('recommended_labels', labels)
            action = ai_validation.get('action', 'none')
            final_title = ai_validation.get('title', title)
            ...
            if token is not None or validate_token(token):
                ...
                if action == 'closed':
                    Pr.objects.create(applicant_id=applicant, repo_id=repo, src_branch=branch, dst_branch=base,
                                      pr_number=output["number"], applicant_name=applicant.name, pr_status=Pr.CLOSED)
                    subprocess.run([
                        "gh", "api",
                        "--method", "PATCH",
                        "-H", f"Authorization: token {token}",
                        f"/repos/{remotePath}/pull/{output['number']}",
                        "-f", "state=closed"
                    ], check=True)
                    response["message"] = "Pr已创建但因不规范被关闭"
                    content = f"您刚刚发布的Pr由于不符合规范被关闭，该Pr属于项目\"{project.name}\"下的\"{repo.name}\"仓库\""
                    msg = Notice.objects.create(receiver_id=applicant, read=Notice.N, content=content)
                    msg.save()
                else:
                    Pr.objects.create(applicant_id=applicant, repo_id=repo, src_branch=branch, dst_branch=base,
            ...
            print("GitHub API调用失败:", e.stderr)
            return JsonResponse(genResponseStateInfo(response, 9, f"API error: {e.stderr}"))
        except json.JSONDecodeError:
            return JsonResponse(genResponseStateInfo(response, 10, "invalid GitHub response"))
        except Exception as e:
            return JsonResponse(genUnexpectedlyErrorInfo(response, e))

    def validate_pr_with_ai(self, title, body, labels):
        try:
            PROMPT_TEMPLATE = """你是一个专业的软件项目管理助手，请对照Github Pull Request规范，分析以下Pull Request：
...
请按以下规则处理：
1. 如果标题不符合规范（缺少类型前缀如[Feat]/[Fix]等，或描述不清晰），返回关闭操作和修改建议
2. 如果内容缺失必要信息（如变更说明、测试方法），返回关闭操作和补充要求
3. 根据内容自动生成相关标签（从现有标签中选择：{existing_labels}），优先选择最相关的3个
4. 输出严格遵守JSON格式：{{"action": "close"|"none", "title": str, "recommended_labels": list}}"""

            existing_labels = ["bug", "enhancement", "documentation", "refactor", "test", "dependencies"]
            ...
            # 验证AI返回格式
            if not isinstance(result, dict) or 'action' not in result or 'title' not in result:
                return {
                    'action': 'none',
                    'title': title,
            ...
            return {
                ...
                'recommended_labels': result.get('recommended_labels', labels),
                'message': result.get('message', '')
            }
        except Exception as e:
            print(f"AI验证失败: {str(e)}")
            return {
                'action': 'none',
                'title': title,
                'recommended_labels': labels,
...
class CreateIssue(View):
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse(response)
        response = {}
        genResponseStateInfo(response, 0, "create issue ok")
        userId = kwargs.get('userId')
        projectId = kwargs.get('projectId')
        repoId = kwargs.get('repoId')
        title = kwargs.get('title')
        ...
        try:
            ...
            return JsonResponse(genResponseStateInfo(response, 4, "repo not found"))
        try:
            user = User.objects.get(id=userId)
            token = user.token
            if token is None or not validate_token(token):
            ...
            return JsonResponse(genResponseStateInfo(response, 6, "user not found"))
        try:
            ai_validation = self.validate_issue_with_ai(title, body, labels)
            final_labels = ai_validation.get('recommended_labels', labels)
            action=ai_validation.get('action', 'none')
            final_title=ai_validation.get('title', title)
            remotePath = repo.remote_path
            ...
            if action == 'close':
                ...
                "--method", "PATCH",
                "-H", f"Authorization: token {token}",
                f"/repos/{remotePath}/issues/{issue_data['number']}",
                "-f", "state=closed"
                ], check=True)
                response["message"] = "Issue已创建但因不规范被关闭"
                content = f"您刚刚发布的issue由于不符合规范被关闭，该issue属于项目\"{project.name}\"下的\"{repo.name}\"仓库\""
    ...
    def validate_issue_with_ai(self, title, body,labels):
        try:
            PROMPT_TEMPLATE = """你是一个专业的软件项目管理助手，请对照Github标准，分析以下Issue：
...
请按以下规则处理：
1. 如果标题和内容叙述简略或不符合issue规范，返回关闭操作和提醒信息
2. 如果标题和内容完整，则根据标题和内容生成最相关的标签（从现有标签中选择：{existing_labels}，该标签集来自Github），如果用户自定义标签与内容关系密切，也可保留用户所选标签
3. 标题必须包含类似于[BUG]/[FEATURE]/[DOCS]等前缀（正则：^$$(BUG|FEATURE|DOCS)$$.+），可根据你生成的标签自定义其他前缀，若原来标题没有（不能只因为这一点就关闭issue），请添加，若原来标题有但和内容不符，请删除原来的前缀并添加新的前缀
3. 输出严格遵守JSON格式：{{"is_valid": bool, "action": "close"|"none", "recommended_labels": [], "message": str, "title": str}}"""

            existing_labels = ["bug", "help wanted", "good first issue", "enhancement", "documentation", "duplicate", "wontfix", "question", "invalid"]
            prompt = PROMPT_TEMPLATE.format(title=title, body=body, labels=labels, existing_labels=existing_labels)
            headers = {"Authorization": "Bearer 84e67deb292945428711e34beb999645.beRfuJzKAFOciVhM"}
            payload = {
            "model": "glm-4-plus",
            "messages": [{"role": "user", "content": prompt}],
            ...
            print(f"AI验证失败: {str(e)}")
            return {'recommended_labels': labels, 'action': 'none', 'title': title}
...
class GetIssueDetails(View):
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse(response)
        genResponseStateInfo(response, 0, "get issue details ok")
        ...
        if token is None or validate_token(token) == False:
            return JsonResponse(genResponseStateInfo(response, 6, "invalid token"))
        path_parts = repo.remote_path.split("/")
        ...
        try:
            cmd = [
                ...
                "-H", "X-GitHub-Api-Version: 2022-11-28",
                "-H", f"Authorization: token {token}",
                f"/repos/{owner}/{repo_name}/issues/{issueId}"
            ]
            result = subprocess.run(cmd, capture_output=True, text=True, check=True)
            issue_data = json.loads(result.stdout)
            ...
            data = {
                ...
                "createdTime": issue_data["created_at"],
                "updatedTime": issue_data["updated_at"]
            }
            response["data"] = data
            return JsonResponse(response)
        except subprocess.CalledProcessError as e:
            print("GitHub API调用失败:", e.stderr)
            return JsonResponse(genResponseStateInfo(response, 7, f"GitHub API error: {e.stderr}"))
        except KeyError as e:
            print("响应数据缺少关键字段:", e)
            return JsonResponse(genResponseStateInfo(response, 8, f"Missing field in response: {e}"))
        except Exception as e:
            return JsonResponse(genUnexpectedlyErrorInfo(response, e))

class CloseIssue(View):
    def post(self, request):
        DBG("---- in " + sys._getframe().f_code.co_name + " ----")
        response = {'message': "404 not success", "errcode": -1}
        try:
            ...
            return JsonResponse(response)
        genResponseStateInfo(response, 0, "close issue ok")
        userId = kwargs.get('userId')
        projectId = kwargs.get('projectId')
        repoId = kwargs.get('repoId')
        issueId = kwargs.get('issueId')
        ...
        try:
            cmd=[
                ...
                "-H", "Accept: application/vnd.github+json",
                "-H", f"Authorization: token {token}",
                f"/repos/{repo.remote_path}/issues/{issueId}",
                "-f", "state=closed"
            ]
            result = subprocess.run(cmd, capture_output=True, text=True, check=True)
            return JsonResponse(response)
        except subprocess.CalledProcessError as e:
            print("API调用失败:", e.stderr)
            return JsonResponse(genResponseStateInfo(response, 7, f"API error: {e.stderr}"))
        except Exception as e:
            return JsonResponse(genUnexpectedlyErrorInfo(response, e))
...
class UpdateIssue(View):
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse(response)
        genResponseStateInfo(response, 0, "update issue ok")
        userId = kwargs.get('userId')
        projectId = kwargs.get('projectId')
        repoId = kwargs.get('repoId')
        issueId = kwargs.get('issueId')
        ...
        try:
            ...
            return JsonResponse(genResponseStateInfo(response, 8, "invalid response"))
        except Exception as e:
            return JsonResponse(genUnexpectedlyErrorInfo(response, e))
