...
export default {
  ...
  methods: {
    async commitToChat() {
      try {
        const comName = this.branchName + "分支中的提交" + this.commitDetails.commit_message;
        const {name, description} = discussionService.generateRoomInfo('commit', {
          id: comName,
    ...
    async confirmDiscussion() {
      try {
        ...
        const preMess = {
          type: this.discussionForm.type,
          projectName: `id=${this.projId}的项目中提交到分支${this.branchName}`,
          name: this.commitDetails.commit_message,
          str: `http://114.116.211.54/commitReview/${this.projId}/${this.repoId}/${this.branchName}/${this.commitSha}?branchName=${this.branchName}&projId=${this.projId}&repoId=${this.repoId}&commitSha=${this.commitSha}` 
        };
...
<template>
  ...
  <div style="margin: 15px" v-else>
    ...
    <el-card style="margin-top: 15px; margin-bottom: 15px">
      ...
      <v-chip :color="'red'" dark style="margin-right: 30px" @click="reviewCommit(0)">
        拒绝
      </v-chip>
      <v-chip :color="'blue'" dark @click="commitToChat">
        讨论发起
      </v-chip>
      </div>
    </el-card>
