...
import FileView from "../views/dev/File.vue"
...
export default {
  ...
  data() {
    ...
    return {
      repo: computed(() => this.selectedRepo),
      branchName: computed(() => this.branches[this.selectedBranchIndex]?.name),
      selectedBranch: computed(() => this.branches[this.selectedBranchIndex]),
      projectId: computed(() => this.proj.id),
      repoId: computed(() => this.selectedRepo.id)
    }
  }, created() {
      this.updateBranches()
  }
